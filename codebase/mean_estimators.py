...
# (dx = 5)
# g = lambda x: jnp.array([x[0] + 10 * x[3] + jnp.sin(x[2]*x[1]) * x[4]])

# Generate Data

# np.random.seed(seed=1)
m = np.ones(dx)
P = np.multiply(np.diag(10.0 * np.random.rand(dx)), np.eye(dx))
...
for p in [11]:  # range(11, 12):
    ...
    Delta = p * 0.1 * P
    if p == 11:
        Delta_opt = utils.sdp_opt(dx, N, 1.0, P, P, hess_m, 10, 0.01)
        Delta = Delta_opt
    ...
    for trial in range(N_trials):
        ...
        Mn = 3
        random.seed(int(100 * random.rand()))
        mc2_sample = random.multivariate_normal(m, P - Delta, Nz)
        for sample in mc2_sample:
            ...
            mc2_estimates[trial] += np.sum(trans_mc2_sample) / N ** 2
    ...
    print('I_MCQ MSE', (simple_quadratic_estimate - baseline_estimate)**2 )
    print(' ** smart Quadratic **')
    print('I_Q', float(smart_quadratic_estimate))
    print('I_MCQ MSE', (smart_quadratic_estimate - baseline_estimate)**2 )
    print('-' * 20)
    if p == 11:
